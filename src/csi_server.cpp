...
int main(int argc, char* argv[]){
  ...
  char subnet[20];
  bool scan=false;
  if(std::regex_search(cfg_rx_ip,ip_match,ip_ex)){
  ...
  std::stringstream IPs(sh_exec_block("g_dev_hostname -I"));
  std::string IP;
  std::string hostIP;
  bool iface_up = false;
  ...
  char setupcmd[512];
  sprintf(setupcmd, "ping -c 3 -i 0.3 %s", cfg_rx_ip.c_str());
  ...
  if(ping_result.find("Destination Host Unreachable",0) != std::string::npos || ping_result.find("100% packet loss",0) != std::string::npos){
	ROS_ERROR("The host at %s did not respond to a ping.", cfg_rx_ip.c_str());
  ...
  if(cfg_do_not_start_nexmon){ROS_WARN("Not configuring the router.");}
  else{
  ...
  if(beacon > 0) {
	...
	sprintf(setupcmd, "sshpass -p %s ssh -o strictHostKeyChecking=no %s@%s /jffs/csi/send.sh %d %d %d %s 11 11 11 %x %x %x > /dev/null 2>&1",
            cfg_rx_pass.c_str(), cfg_rx_host.c_str(), cfg_rx_ip.c_str(), bw, tx_nss,
  ...
  char topic_name[256];
  // std::string rx_no_dot(cfg_rx_ip);
  ...
  // Create socket
  if (cfg_use_tcp) {//forward over tcpdump-netcat
  ...
  if(cfg_use_tcp){
    ...
    while(ros::ok()) {
	  ...
	  if (connfd == -1 && errno == EAGAIN) {
		...
		ROS_INFO("Accepted Connection from %s", cfg_rx_ip.c_str());
		break;
	  }
	  else{
...
void publish_csi(std::vector<csi_instance> &channel_current){
  ...
  ROS_INFO("%s:RSSI%d/seq%d/fc%.2hhx/chan%d/rx%s",hr_mac(csi_0.source_mac).c_str(), msgout.rssi, msgout.seq_num, csi_0.fc, msgout.chan, cfg_rx_ip.c_str());
  memset(csi_r_out,0,num_floats*sizeof(double));
  memset(csi_i_out,0,num_floats*sizeof(double));
  msgout.rx_id = cfg_rx_ip;
  msgout.msg_id = 0;
...
void handle_shutdown(int sig){
  ...
  if(tx_fp){
	...
	sprintf(killcmd, "sshpass -p %s ssh -o strictHostKeyChecking=no %s@%s killall send.sh", cfg_rx_pass.c_str(), cfg_rx_host.c_str(), cfg_rx_ip.c_str());
...
std::string reconfigure(){
  ...
  if(filter.len > 1){
	sprintf(configcmd, "sshpass -p %s ssh -o strictHostKeyChecking=no %s@%s /jffs/csi/setup.sh %d %d 4 %.2hhx:%.2hhx:00:00:00:00 2>&1",
            cfg_rx_pass.c_str(), cfg_rx_host.c_str(), cfg_rx_ip.c_str(), ch, bw, filter.mac[0],filter.mac[1]);
  }
  else{
	sprintf(configcmd, "sshpass -p %s ssh -o strictHostKeyChecking=no %s@%s /jffs/csi/setup.sh %d %d 4 2>&1", cfg_rx_pass.c_str(), cfg_rx_host.c_str(), cfg_rx_ip.c_str(), ch, bw);
  }
...
void setup_tcpdump(std::string hostIP){
  char setupcmd[512];
  sprintf(setupcmd, "sshpass -p %s ssh -o strictHostKeyChecking=no %s@%s /jffs/csi/tcpdump -i %s port 5500 -nn -s 0 -w - --immediate-mode | nc %s %d > /dev/null 2>&1", cfg_rx_pass.c_str(), cfg_rx_host.c_str(), cfg_rx_ip.c_str(), iface.c_str(), hostIP.c_str(), PORT_TCP);
  ROS_INFO("%s",setupcmd);
...
void setup_params(ros::NodeHandle& nh){
  ...
  nh.param<bool>("tcp_forward", cfg_use_tcp, false);
  nh.param<std::string>("asus_ip", cfg_rx_ip, "");
  nh.param<std::string>("asus_pwd", cfg_rx_pass, "password");
  nh.param<std::string>("asus_host", cfg_rx_host, "HOST");
